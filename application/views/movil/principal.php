...
for ($i = 0; $i < $numGrids + 2; $i = $i + 2) {
    ...
    echo '<a href="' . site_url('movil/categorias/ver/' . $comercio->categorias[$i]->id) . '"><img src="' . base_url($comercio->categorias[$i]->icono) . '" width="200" height="200"/></a>';
    echo '<div class="nombre-categoria">' . $comercio->categorias[$i]->nombre . '</div>';
    echo '</div>';
    echo '<div class="ui-block-b">';
    echo '<a href="' . site_url('movil/categorias/ver/' . $comercio->categorias[$i + 1]->id) . '"><img src="' . base_url($comercio->categorias[$i + 1]->icono) . '" width="200" height="200"/></a>';

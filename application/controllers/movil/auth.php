        ...
        try {
            ...
            if ($this->hybridauthlib->providerEnabled($provider)) {
                ...
                if ($service->isUserConnected()) {
                    ...
                    // Construye usuario on user profile y guarda en sesión, falta ver si es primer registro o usuario existente para todos
                    $cliente = NULL;
                    if ($provider === 'Facebook') {
                        // load cliente, si no existe lo agrega
                        $cliente = new Cliente();
                        $cliente->idFacebook = $user_profile->identifier;
                        $cliente->nombre = $user_profile->firstName . ' ' . $user_profile->lastName;
                        $cliente->mail = $user_profile->email;
                        $cliente->metodoAut = 'fb';
                    } else if ($provider === 'Twitter') {
                        // load cliente, si no existe lo agrega
                        $cliente = new Cliente();
                        $cliente->idTwitter = $user_profile->identifier;
                        $cliente->nombre = $user_profile->firstName . ' ' . $user_profile->lastName;
                        $cliente->metodoAut = 'tw';
                    } else if ($provider === 'Google') {
                        // load cliente, si no existe lo agrega
                        $cliente = new Cliente();
                        $cliente->idGoogle = $user_profile->identifier;
                        $cliente->nombre = $user_profile->firstName . ' ' . $user_profile->lastName;
                        $cliente->metodoAut = 'go';
                    }
                    $data['cliente'] = $cliente;
                    $data['titulo'] = 'Mis datos';
                    $this->nativesession->set('cliente', $cliente);
                    // Autocomplete
